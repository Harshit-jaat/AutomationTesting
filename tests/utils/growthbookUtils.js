      ...
      logcat.stdout.on("data", (data) => {
          ...
          lines.forEach((line) => {
              if (line.includes("roleeeeeee")) {
                  ...
                  // Match pattern: 'roleeeeeee', 'Teacher', 'Base'
                  const match = line.match(/'roleeeeeee',\s*'(\w+)',\s*'(\w+)'/i);

                  if (match) {
                      const role = match[1].toLowerCase();     // e.g. 'teacher'
                      const variant = match[2].toLowerCase();  // e.g. 'base' or 'a'

                      clearTimeout(timeout);
                      logcat.kill();

                      broadcastMessage(`🧪 Role: ${role}, Variant: ${variant}`);
                      resolve({ role, variant });
                  }
              }
          });
      });
